    ...
    render(
        html`
            <div class="container-fluid">
                ...
                <button id="btn-change-branch" type="button" class="btn btn-sm btn-outline-dark float-right ms-3 mt-3 mb-3" style="width:50px">ganti</button>
                <hr />
                <p id="data-label" class="text-center bg-light shadow rounded py-3 px-3">Transaksi Hari Ini</p>
                <div class="container-fluid">
                    <button id="add-transaction" type="button" class="btn btn-sm bgs-bluelight float-right mt-3 mb-3" style="width:50px"><i class="fa fa-plus" aria-hidden="true"></i></button>
                    <button id="edit-transaction" type="button" class="btn btn-sm bgs-bluelight float-right mt-3 mb-3" style="width:50px">
                        <i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                    </button>
                    <button id="delete-transaction" type="button" class="btn btn-sm bgs-redlight float-right mt-3 mb-3" style="width:50px"><i class="fa fa-minus" aria-hidden="true"></i></button>
                    <button id="filter-transaction" type="button" class="btn btn-sm bgs-bluelight float-right mt-3 mb-3" style="width:50px"><i class="fa fa-filter" aria-hidden="true"></i></button>
                    <button id="print-struct-transaction" type="button" class="btn btn-sm bgs-bluelight float-right mt-3 mb-3" style="width:50px">
...
async function setTransactionData(url, dataparam) {
    ...
    if (w > h) {
        ...
        render(html` <table id="table-data" class="table" style="width:100%"></table> `, $("#table-container")[0]);
    }
    ...
    if (w > h) {
        ...
        columnlayout = [{ title: "Transaksi", data: "onecolumn", width: "100%" }];
        // columnlayout = [
            ...
            dataSrc: function (result) {
                ...
                for (let index = 0; index < dataset.length; index++) {
                    ...
                    element.cr_time_view =
                        '<i class="fa fa-calendar" aria-hidden="true"></i> ' + element.cr_time.substring(0, 16).replace("T", ' <br> <i class="fa fa-clock-o" aria-hidden="true"></i> ');
                    element.mod_time_view =
                        '<i class="fa fa-calendar" aria-hidden="true"></i> ' + element.mod_time.substring(0, 16).replace("T", ' <br> <i class="fa fa-clock-o" aria-hidden="true"></i> ');
                    if (element.paid_date)
                        element.paid_date_view =
                            '<i class="fa fa-calendar" aria-hidden="true"></i> ' + element.paid_date.substring(0, 16).replace("T", ' <br> <i class="fa fa-clock-o" aria-hidden="true"></i> ');
                    else element.paid_date_view = `<i class="fa fa-times" aria-hidden="true"></i>`;
                    if (element.finish_date)
                        element.finish_date_view =
                            '<i class="fa fa-calendar" aria-hidden="true"></i> ' + element.finish_date.substring(0, 16).replace("T", ' <br> <i class="fa fa-clock-o" aria-hidden="true"></i> ');
                    ...
                    element.transaction_view = "ID: " + element.id + "<br>" + element.cr_time_view + "<br>" + element.status_view + element.paid_date_view + "<br>" + element.payment_view;
                    element.customer_view = element.data.customername + "<br>" + element.customer + "<br>" + element.data.kg + " Kg<br>" + "Rp " + element.amount + "<br>";
        ...
        html`
            <div class="d-flex p-2 bd-highlight justify-content-center">
                <div id="struct-print" class="text-center border py-2 px-2" style="width: 300px;">
                    ...
                    ${data[0].data.discont !== "0" ? "Diskon " + data[0].data.discont + "% dari harga asli Rp." + data[0].data.realamount : ""}
                    <br /><br />
                    Bayar (Rp)<br />
                    ...
                    if (data.length > 0) {
                        ...
                        render(
                            html`
                                ${data.map(
                                    (item) =>
                                        html`<button type="button" class="list-group-item list-group-item-action selectednumber" value="${item.phone_number}">
                                            ${item.phone_number} - ${item.fullname}
                                        </button>`
                                )}
                            `,
                            $("#numberlist")[0]
...
async function inputNewTransaction(phone_number, isNewMember) {
    ...
    console.log("number " + result[0].phone_number);
    ...
    var trx_fullname = "";
    var trx_phone_number = "";
    var trx_address = "";
    if (isNewMember) {
        if (isNaN(phone_number)) {
            trx_fullname = phone_number;
        } else {
            trx_phone_number = phone_number;
        }
    } else {
        trx_fullname = result[0].fullname;
        trx_phone_number = result[0].phone_number;
        trx_address = result[0].address;
    }
    ...
    swal.showModal(
        ...
        html`
            ...
            <div class="form-floating mb-3">
                <input type="text" class="form-control" id="input-customer-number" value="${trx_phone_number}" />
            ...
            <div class="form-floating mb-3">
                <input type="text" class="form-control" id="input-customer-name" value="${trx_fullname}" />
    ...
    $("#trx-random-phone").on("click", function () {
            var cd = new Date();
            ...
            $("#input-customer-number").val(randomphone);
    });
...
async function editTransaction(data) {
    ...
    swal.showModal(
        ...
        html`
            ...
            <!-- <div class="form-floating mb-3">
                                                                                                                                                                    <input disabled type="text" class="form-control" id="input-product-name" value="${data[0]
                .product_view}">
